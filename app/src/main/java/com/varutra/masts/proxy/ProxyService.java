...
import com.varutra.masts.proxy.ShellCommand.CommandResult;

public class ProxyService extends PreferenceActivity {
	public static final String PREFS_NAME = "prefs";
	private static final int REDSOCKS_HTTP_PORT = 8123;
	private static final int REDSOCKS_HTTPS_PORT = 8124;
	private static final String LOGTAG = ProxyService.class.getSimpleName();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		findPreference("version").setSummary("Varutra Proxy "+getVersion(this));
	...
	public boolean proxy(int action) {
		...
		if (action == START) { // start proxy
			...
			host = settings.getString("proxyHost", "");
			port = settings.getString("proxyPort", "");
			auth = settings.getBoolean("isAuthEnabled", false);
			...
			user = settings.getString("username", "").replace("\\", "\\\\");
			Log.d("VPROXY", "user:"+user);
			pass = settings.getString("password", "");

			String ipaddr;
			String arch = System.getProperty("os.arch");
			Log.d("Vproxy", "arch:"+arch);
			...
			try {
				Log.d("VPROXY", "CONF FILE: "+basedir);
				File f = new File(basedir, "redsocks.conf");
				f.createNewFile();
				Log.d("VPROXY", "EXISTS"+f.exists());
	...
	static boolean startRedsocks(String arch, String basedir) {
		...
		CommandResult r = cmd.sh.runWaitFor(basedir + "/proxy.sh start " + basedir + " " + "http" + " " + host + " " + port + " " + auth + " " + pass + " " + user);//cmd.sh.runWaitFor(basedir+"/redsocks-"+arch+" -p "+basedir+"/"+REDSOCK_PID_FILE+" -c "+basedir+"/redsocks.conf" );
		if (!r.success()) {
			Log.v("Vproxy", "Error starting proxy.sh (" + r.stderr + ")");
	...
	static boolean applyIptables(String basedir) {
		ShellCommand cmd = new ShellCommand();
		CommandResult r = cmd.su.runWaitFor(basedir+"/redirect.sh start http");
		if (!r.success()) {
			Log.v("vproxy", "Erro" +
					"r starting redirect.sh (" + r.stderr +")");
			//todo kill redsocks process
			Log.e(LOGTAG, "Failed to start redirect.sh ("+ r.stderr + ")", null);
			return false;
		} else {
			Log.v("vproxy", "Successfully ran redirect.sh start ");
			return true;
		}
	}
