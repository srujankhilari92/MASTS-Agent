...
import com.varutra.webscarab.util.Glob;
public class URLFetcher implements HTTPClient {
    ...
    public Response fetchResponse(Request request) throws IOException {
        ...
        if (request == null) {
        	Log.e("URLFetcher", "Asked to fetch a null request");
            _logger.severe("Asked to fetch a null request");
            return null;
        }
        HttpUrl url = request.getURL();
        if (url == null) {
        	Log.e("URLFetcher", "Asked to fetch a request with a null URL");
    ...
    private boolean invalidSocket(HttpUrl url) {
        if (_host == null || _in == null) return true;
